...
static void ADS1256_DelayDATA(void)
{
	...
	*/
	bsp_DelayUS(5.5);	/* The minimum time delay 6.5us */
}
...
void start_ADC(int gain, int sampling_rate, int scan_mode) {
	...
	g_tADS1256.Gain = static_cast<ADS1256_GAIN_E>(gain);
	g_tADS1256.DataRate = static_cast<ADS1256_DRATE_E>(sampling_rate);
	// setup ADC gain and sampling rate
	ADS1256_CfgADC(g_tADS1256.Gain, g_tADS1256.DataRate);
...
double *readdata(int Length)
{
	int length = 1000;
	double adc[8];
	cout <<"start wait data"<<endl;
	double volts[8];
...
PyObject *read_channel_volts(int channel,int Length) {
	...
	PyObject *ret;
	ret = Convert_big_Array(data,length);
...
void read_all_channels_volts(double * volts, int ch) {
	...
	for (i = 0; i < ch; i++) {
		// 5 volts reference divided by 2^23
		// 24 bits ranging from +5v to -5v
		volts[i] = ((double) ADS1256_GetAdc(i) * 5 / 8388608)+100;
	}
}
/*
void read_buffer_volts(int channel){
